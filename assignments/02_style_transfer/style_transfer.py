...
class StyleTransfer(object):
    def __init__(self, content_img, style_img, img_width, img_height):
        ...
        self.initial_img = utils.generate_noise_image(
            self.content_img, img_width, img_height
        )
    ...
    def create_input(self):
        ...
        with tf.variable_scope("input"):
            self.input_img = tf.get_variable(
                "in_img",
    ...
    def _content_loss(self, P, F):
        ...
        ###############################
        ## TO DO
        self.content_loss = tf.reduce_sum((F - P) ** 2 / (4 * P.size))
        ###############################
    ...
    def losses(self):
        with tf.variable_scope("losses"):
            ...
            with tf.Session() as sess:
                ...
                style_layers = sess.run(
                    [getattr(self.vgg, layer) for layer in self.style_layers]
                )
            ...
            self.total_loss = (
                self.content_w * self.content_loss + self.style_w * self.style_loss
            )
            ##########################################
    ...
    def optimize(self):
        ...
        self.opt = tf.train.AdamOptimizer(self.lr).minimize(
    ...
    def train(self, n_iters):
        ...
        with tf.Session() as sess:
            ...
            ckpt = tf.train.get_checkpoint_state(
                os.path.dirname("checkpoints/style_transfer/checkpoint")
            )
            ...
            for index in range(initial_step, n_iters):
                ...
                if (index + 1) % skip_step == 0:
                    ...
                    if (index + 1) % 20 == 0:
                        ...
                        saver.save(
                            sess, "checkpoints/style_stranfer/style_transfer", index
                        )
                        ###############################
                        pass
...
if __name__ == "__main__":
    setup()
    machine = StyleTransfer(
        "/home/ubuntu/workspace/namtd/deep-learning/cs20/assignments/02_style_transfer/content/deadpool.jpg",
        "/home/ubuntu/workspace/namtd/deep-learning/cs20/assignments/02_style_transfer/styles/vangosh.jpg",
        333,
        250,
    )
    machine.build()
