""" Assigment 1 - Problem 4: CBOW model.
"""
import os
...
class CbowModel:
    ...
    def __init__(
        self, dataset, vocab_size, embed_size, batch_size, num_sampled, learning_rate
    ):
    ...
    def _import_data(self):
        ...
        with tf.name_scope("data"):
            ...
            self.center_words = tf.expand_dims(self.center_words, axis=-1)
            self.target_words = tf.squeeze(self.target_words)
    ...
    def _create_embedding(self):
        ...
        with tf.name_scope("embed"):
            ...
            self.embed = tf.nn.embedding_lookup(
                self.embed_matrix, self.target_words, name="embedding"
            )
...
def gen():
    yield from word2vec_utils.batch_gen(
        DOWNLOAD_URL, EXPECTED_BYTES, VOCAB_SIZE, BATCH_SIZE, SKIP_WINDOW, VISUAL_FLD
    )
...
def main():
    ...
    model = CbowModel(
        dataset, VOCAB_SIZE, EMBED_SIZE, BATCH_SIZE, NUM_SAMPLED, LEARNING_RATE
    )
    model.build_graph()
